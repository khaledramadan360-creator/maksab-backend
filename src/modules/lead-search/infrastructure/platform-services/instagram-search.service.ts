...
export class InstagramSearchService {
  ...
  public async search(request: SearchRequest): Promise<PlatformSearchResult> {
    ...
    return {
      platform: SearchPlatform.INSTAGRAM,
      requestedCount: request.requestedResultsCount,
  ...
  private extractLightweightCandidate(raw: RawSearchResult): CandidateResult | null {
    const canonicalUrl = this.normalizer.normalize(raw.url, SearchPlatform.INSTAGRAM);
    if (!canonicalUrl) {
      return null;
    }
  ...
  private extractName(title: string): string {
    const cleanedTitle = title
      ...
      .replace(/\(@[^)]+\)/g, '')
      .replace(/\s*[^a-z0-9\u0600-\u06FF]+\s*instagram photos and videos$/i, '')
      .trim();
  ...
  private buildWarning(
    ...
    if (!sawAnyRawResults) {
      return providerWarnings[0] || 'Exhausted all Instagram query/page combinations before collecting candidates.';
    }

    if (stoppedAtFilterThreshold) {
      return `Stopped Instagram filtering after reaching the shared acceptance threshold of ${filterStopTarget} results.`;
    }

    if (acceptedDuringFiltering === 0) {
      return `Fetched ${rawResultCount} raw Instagram SERP results across ${queriedPageCount} pages, but none passed the platform, client-only, and relevance filters.`;
    }

    if (providerWarnings.length > 0) {
      return `Returned ${finalCount} Instagram results after scanning ${queriedPageCount} pages, but some query/page requests failed during collection.`;
    }

    return `Fetched ${rawResultCount} raw Instagram SERP results, accepted ${acceptedDuringFiltering} after filtering, and returned ${finalCount} best results.`;
  }
}
