...
export class FacebookSearchService {
  ...
  public async search(request: SearchRequest): Promise<PlatformSearchResult> {
    ...
    return {
      platform: SearchPlatform.FACEBOOK,
      requestedCount: request.requestedResultsCount,
  ...
  private extractLightweightCandidate(raw: RawSearchResult): CandidateResult | null {
    const canonicalUrl = this.normalizer.normalize(raw.url, SearchPlatform.FACEBOOK);
    if (!canonicalUrl) {
      return null;
    }
    ...
    const resultType =
      canonicalUrl.includes('/pages/') ||
      canonicalUrl.includes('/business/') ||
      combinedLower.includes('page') ||
      this.matchesAny(combinedLower, ['official', 'company', 'clinic', 'agency', 'restaurant', 'store', 'شركة', 'عيادة', 'مؤسسة'])
        ? ResultType.OFFICIAL_BUSINESS_PAGE
  ...
  private buildWarning(
    ...
    if (!sawAnyRawResults) {
      return providerWarnings[0] || 'Exhausted all Facebook query/page combinations before collecting candidates.';
    }

    if (stoppedAtFilterThreshold) {
      return `Stopped Facebook filtering after reaching the shared acceptance threshold of ${filterStopTarget} results.`;
    }

    if (acceptedDuringFiltering === 0) {
      return `Fetched ${rawResultCount} raw Facebook SERP results across ${queriedPageCount} pages, but none passed the platform, client-only, and relevance filters.`;
    }

    if (providerWarnings.length > 0) {
      return `Returned ${finalCount} Facebook results after scanning ${queriedPageCount} pages, but some query/page requests failed during collection.`;
    }

    return `Fetched ${rawResultCount} raw Facebook SERP results, accepted ${acceptedDuringFiltering} after filtering, and returned ${finalCount} best results.`;
  }
}
