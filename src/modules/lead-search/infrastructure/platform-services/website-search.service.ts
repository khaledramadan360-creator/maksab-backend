...
export class WebsiteSearchService {
  ...
  private extractLightweightCandidate(raw: RawSearchResult): CandidateResult | null {
    const canonicalUrl = this.normalizer.normalize(raw.url, SearchPlatform.WEBSITE);
    if (!canonicalUrl) {
      return null;
    }
    ...
    const resultType =
      this.looksLikeHomepage(canonicalUrl) ||
      titleLower.includes('official site') ||
      titleLower.includes('homepage') ||
      snippetLower.includes('official website')
  ...
  private buildWarning(
    ...
    if (!sawAnyRawResults) {
      return providerWarnings[0] || 'Exhausted all website query/page combinations before collecting candidates.';
    }

    if (stoppedAtFilterThreshold) {
      return `Stopped website filtering after reaching the shared acceptance threshold of ${filterStopTarget} results.`;
    }

    if (acceptedDuringFiltering === 0) {
      return `Fetched ${rawResultCount} raw website SERP results across ${queriedPageCount} pages, but none passed the platform, client-only, and relevance filters.`;
    }

    if (providerWarnings.length > 0) {
      return `Returned ${finalCount} website results after scanning ${queriedPageCount} pages, but some query/page requests failed during collection.`;
    }

    return `Fetched ${rawResultCount} raw website SERP results, accepted ${acceptedDuringFiltering} after filtering, and returned ${finalCount} best results.`;
  }
}
