...
export class FacebookQueryBuilder implements SearchQueryBuilder {
  ...
  public buildQueries(request: SearchRequest): SearchQueryVariant[] {
    ...
    for (const pattern of prepared.patterns) {
      ...
      const finalQuery = ['site:facebook.com', pattern.queryText, ...locationParts].filter(Boolean).join(' ').trim();
