...
export class InstagramQueryBuilder implements SearchQueryBuilder {
  ...
  public buildQueries(request: SearchRequest): SearchQueryVariant[] {
    ...
    for (const pattern of prepared.patterns) {
      ...
      const finalQuery = ['site:instagram.com', pattern.queryText, ...locationParts].filter(Boolean).join(' ').trim();
